...
fn contributor(gate_median: Option<f32>) -> recalld::room::Contributor {
    recalld::room::Contributor {
...
/// ⚠ A source with too little history is UNRANKABLE, never condemned. The
/// opposite default would drop every source on the day it is added, which is
/// exactly when nothing is known about it.
#[test]
fn a_source_without_a_signature_is_not_judged_to_be_gating() {
    assert!(!recalld::room::is_gating(&contributor(None)));
}

/// The fleet's measured medians, either side of the cut: geb before its capsule
/// was swapped against the loudest healthy source in the same sample.
#[test]
fn the_cut_separates_the_gating_device_from_the_loudest_healthy_source() {
    assert!(
        recalld::room::is_gating(&contributor(Some(2.830))),
        "geb pre-swap, median 2.830 s"
    );
    assert!(
        !recalld::room::is_gating(&contributor(Some(0.226))),
        "pixel5, median 0.226 s — the highest healthy median measured"
    );
}

/// ⚠ A repaired device must be able to come back. geb's quiet runs went to zero
/// when its capsule was swapped, and a rule keyed on a rolling window readmits
/// it; one keyed on all history never would.
#[test]
fn a_repaired_source_stops_reading_as_gating() {
    assert!(!recalld::room::is_gating(&contributor(Some(0.0))));
}

/// ⚠ The signature is computed by SQL, and the arithmetic tests above cannot
/// reach it. This one goes through the real query: a source with a history of
/// long quiet runs during speech must record a median above the cut, and it
/// must be RECORDED rather than acted on — the block still builds normally.
#[test]
fn a_gating_sources_signature_is_recorded_without_changing_the_verdict() {
    let dir = tempfile::tempdir().expect("tmp");
    for i in 0..4 {
        stored(dir.path(), "usb", &format!("20260905T1000{i:02}"), 0.5);
    }
    ...
    scan_once(dir.path(), 100).expect("levels");
    seed_speech(dir.path());
    // Give every clip the quiet-run history of a gating device.
    let conn = store::open(dir.path()).expect("db");
    conn.execute("UPDATE segment_levels SET quiet_run_s = 2.830", [])
        .expect("seed runs");
    drop(conn);
    ...
    assert!(
        contributors.contains("\"gate_median\":2.83"),
        "the signature is persisted as provenance, got {contributors}"
    );
    assert!(
        verdict.starts_with("built"),
        "⚠ the rule is OFF: recording it must not drop the source, got {verdict}"
    );
}

/// …and the same block with too little history records no signature at all,
/// which is what keeps a new source from being condemned on its first day.
#[test]
fn a_source_with_too_little_history_records_no_signature() {
    let dir = tempfile::tempdir().expect("tmp");
    stored(dir.path(), "usb", "20260905T110000", 0.5);
    let block = DateTime::parse_from_rfc3339("2026-09-05T11:00:00Z")
        .expect("t")
    ...
    let contributors: String = conn
        ...
        .expect("row");
    assert!(
        contributors.contains("\"gate_median\":null"),
        "one clip is below min_reference_rows, got {contributors}"
    );
}
