...
/// `serde_json` writes `{"s":0}`; `json.dumps` writes `{"s": 0}`. Every stored
...
/// Serialise word timings exactly as the Python writes them.
pub fn dump_words(words: &[Word]) -> String {
    let mut out = Vec::new();
    let mut ser = serde_json::Serializer::with_formatter(&mut out, PythonJson);
    serde::Serialize::serialize(words, &mut ser).expect("word timings serialise");
...
fn insert_piece(
    ...
    let words = piece.words.as_deref().map(dump_words);
    tx.execute(
