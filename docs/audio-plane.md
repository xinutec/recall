...
exist in phase-intact form. That was once only in RAM at ingest, because the
archive was Opus 32 kbps voip, which does not preserve phase. **Lossless is the
segmenter's default from 2026-09-11**, so every network mic is phase-intact at
rest and kept forever ([architecture.md](architecture.md)); `iphone11` is the
one still-lossy source, at its own end. The Opus era stays un-fusable — that is
an information fact, not a precision one (#1522).
