...
_MIGRATIONS: tuple[str, ...] = (
    ...
    """,
    # v33 — what a candidate segment *contains*, as opposed to how loud it is.
    #
    # `speech_s` is the VAD's verdict: seconds of detected speech (0.0 = none), and the
    # veto the cleanup rests on. ⚠ The transcript could not carry that weight — a
    # reprocessing pass hides its predecessor's turns, so a segment of real Dutch was
    # left with *no visible turn at all* and the transcript veto never saw it. The VAD
    # is a trained detector and answers directly. NULL = not yet analysed, and an
    # unanalysed segment is unknown, so it is never swept.
    #
    # `structure` is how far a segment's most unusual moment departs from that mic's own
    # noise fingerprint (`sources.noise_shape`) — the mic's self-noise is stationary,
    # real sound is not. It RANKS the spans, pure dead air above someone shifting on the
    # sofa, but never decides: the distributions overlap and only the VAD votes on
    # speech.
    """
